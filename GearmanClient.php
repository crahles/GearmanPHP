...
class GearmanPHP_GearmanClient
{
    ...
    public function  __call($name,  $arguments)
    {
        ...
        if($name === 'do') {
            return $this->_gmcDo($arguments[0], $arguments[1], $arguments[2]);
        } elseif ($name === 'echo') {
    ...
    protected function _submitTask(GearmanPHP_GearmanTask $task)
    {
        ...
        if (!is_array(GearmanPHP_Base_Common::$waiting[$socket])) {
            GearmanPHP_Base_Common::$waiting[$socket] = array();
        }

        array_push(GearmanPHP_Base_Common::$waiting[$socket], $task);
    }
    ...
    protected function _handleResponse($response, $socket)
    {
        ...
        switch ($response['function']) {
            ...
            case 'WORK_DATA':
                ...
                break;
            case 'JOB_CREATED':
                $task = array_shift(GearmanPHP_Base_Common::$waiting[$socket]);
